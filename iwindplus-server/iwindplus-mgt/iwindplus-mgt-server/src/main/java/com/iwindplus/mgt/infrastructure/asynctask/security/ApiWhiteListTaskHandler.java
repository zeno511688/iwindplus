...
package com.iwindplus.mgt.infrastructure.asynctask.security;
...
public class ApiWhiteListTaskHandler implements AsyncTaskHandler {
    ...
    private void sendMessage(String content) {
        final String topicName = kafkaProperty.listTopic(kafkaProperty.getDefaultCluster()).get(1);
        Message<String> message = MessageBuilder
            .withPayload(content)
            .setHeader(KafkaHeaders.TOPIC, topicName)
            .build();
        this.kafkaTemplateRouter.send(message);
        log.info("API白名单 topic 成功={}", content);
    }
}
