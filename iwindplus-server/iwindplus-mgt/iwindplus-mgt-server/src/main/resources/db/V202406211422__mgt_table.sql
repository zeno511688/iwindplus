...
(
    ...
    `client_id_issued_at`      bigint unsigned  NOT NULL DEFAULT 0 COMMENT '客户端签发时间',
    `client_secret`            varchar(255)     NOT NULL DEFAULT '' COMMENT '客户端密钥',
    `client_secret_expires_at` bigint unsigned  NOT NULL DEFAULT 0 COMMENT '客户端密钥过期时间',
...
(
    ...
    `api_urls`                text                         DEFAULT NULL COMMENT 'API路径集合（菜单固有的列表接口等）',
    `level`                   int(10) unsigned    NOT NULL DEFAULT 1 COMMENT '级别',
    `seq`                     int(10) unsigned    NOT NULL DEFAULT 1 COMMENT '排序号',
    `new_feature_expire_time` bigint(20) unsigned          DEFAULT NULL COMMENT '新功能标记过期时间（为空表示非新功能，当前时间小于过期时间时显示新图标）',
    `hide_flag`               tinyint(3) unsigned NOT NULL DEFAULT 0 COMMENT '是否隐藏（0：否，1：是）',
    `build_in_flag`           tinyint(3) unsigned NOT NULL DEFAULT 0 COMMENT '是否内置（0：否，1：是）',
    `parent_id`               bigint(20) unsigned NOT NULL DEFAULT 0 COMMENT '父类主键',
    `system_id`               bigint(20) unsigned NOT NULL DEFAULT 0 COMMENT '系统主键',
    PRIMARY KEY (`id`),
...
(
    ...
    `code`               varchar(50)         NOT NULL DEFAULT '' COMMENT '编码',
    `name`               varchar(50)         NOT NULL DEFAULT '' COMMENT '名称',
    `api_urls`           text                NOT NULL DEFAULT '' COMMENT 'API路径集合',
    `seq`                int(10) unsigned    NOT NULL DEFAULT 1 COMMENT '排序号',
    `build_in_flag`      tinyint(3) unsigned NOT NULL DEFAULT 0 COMMENT '是否内置（0：否，1：是）',
