...
public class SmsCodeAuthenticationConverter implements AuthenticationConverter {
    ...
    public Authentication convert(HttpServletRequest request) {
        ...
        if (!CharSequenceUtil.equals(grantType, GrantTypeConstant.SMS_CODE.getValue())) {
            return null;
        }
        ...
        // 手机号(必需)
        String mobile = parameters.getFirst(GrantTypeConstant.MOBILE);
        if (CharSequenceUtil.isBlank(mobile)) {
            Oauth2Util.throwError(
                OAuth2ErrorCodes.INVALID_REQUEST,
                GrantTypeConstant.MOBILE,
                Oauth2Util.ERROR_URI);
        }

        // 验证码(必需)
        String captcha = parameters.getFirst(GrantTypeConstant.CAPTCHA);
        if (CharSequenceUtil.isBlank(captcha)) {
            Oauth2Util.throwError(
                OAuth2ErrorCodes.INVALID_REQUEST,
                GrantTypeConstant.CAPTCHA,
                Oauth2Util.ERROR_URI);
        }
