...
public class SmsCodeAuthenticationConverter implements AuthenticationConverter {
    ...
    public Authentication convert(HttpServletRequest request) {
        ...
        if (!CharSequenceUtil.equals(grantType, AuthConstant.GrantTypeSmsCodeConstant.SMS_CODE.getValue())) {
            return null;
        }
        ...
        // 配置编码(必需)
        String code = parameters.getFirst(AuthConstant.GrantTypeSmsCodeConstant.CODE);
        if (CharSequenceUtil.isBlank(code)) {
            Oauth2Util.throwError(
                OAuth2ErrorCodes.INVALID_REQUEST,
                AuthConstant.GrantTypeSmsCodeConstant.CODE,
                Oauth2Util.ERROR_URI);
        }

        // 手机号(必需)
        String mobile = parameters.getFirst(AuthConstant.GrantTypeSmsCodeConstant.MOBILE);
        if (CharSequenceUtil.isBlank(mobile)) {
            Oauth2Util.throwError(
                OAuth2ErrorCodes.INVALID_REQUEST,
                AuthConstant.GrantTypeSmsCodeConstant.MOBILE,
                Oauth2Util.ERROR_URI);
        }
        ...
        parameters.forEach((key, value) -> {
            if (!key.equals(OAuth2ParameterNames.GRANT_TYPE)
                ...
                && !key.equals(AuthConstant.GrantTypeSmsCodeConstant.CODE)
                && !key.equals(AuthConstant.GrantTypeSmsCodeConstant.MOBILE)
                && !key.equals(AuthConstant.GrantTypeSmsCodeConstant.CAPTCHA)) {
