...
import com.iwindplus.setup.server.dal.mapper.SmsConfigMapper;
 ...
 */
@Repository
public class SmsConfigRepository extends JoinCrudRepository<SmsConfigMapper, SmsConfigDO> {
    ...
    public void getNameIsExist(String name, Long orgId) {
        final LambdaQueryWrapper<SmsConfigDO> queryWrapper = Wrappers.lambdaQuery(SmsConfigDO.class)
            .eq(SmsConfigDO::getOrgId, orgId)
            .eq(SmsConfigDO::getName, name);
