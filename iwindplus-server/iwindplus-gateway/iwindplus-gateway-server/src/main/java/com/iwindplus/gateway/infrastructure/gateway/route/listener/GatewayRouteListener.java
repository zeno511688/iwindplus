...
package com.iwindplus.gateway.infrastructure.gateway.route.listener;
...
import com.iwindplus.gateway.infrastructure.configuration.constant.GatewayConstant;
        ...
        LAST_DIGEST.updateAndGet(last -> {
            if (last == null || !Objects.equals(last, digest)) {
                List<RouteDefinition> entities = sorted.stream()
                    ...
                    .filter(Objects::nonNull)
                    .toList();
                gatewayRouteManager.loadRoute(entities);
                log.info("加载路由成功");
                return digest;
            }
