...
import com.iwindplus.gateway.infrastructure.configuration.BaseProperty;
...
public class BaseFilter extends BaseGatewayFilter {
    ...
    @Override
    public int getOrder() {
        return GatewayFilterConstant.FILTER_BASE_ORDER;
    }
    ...
    protected Mono<ServerWebExchange> before(ServerWebExchange exchange) {
        ...
        // 2. cookie → token
        if (Boolean.TRUE.equals(property.getEnabledAuthCookie())) {
        ...
        String path = processPath(request.getPath().value(), prefix, property.getEnabledRelativePath());
