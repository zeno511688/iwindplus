...
import com.iwindplus.gateway.infrastructure.client.vo.ApiWhiteListChangeVO;
import com.iwindplus.gateway.infrastructure.configuration.property.ApiWhiteListProperty;
import com.iwindplus.gateway.infrastructure.configuration.constant.GatewayCacheConstant;
...
public class ApiWhiteListFilter extends BaseGatewayFilter {
    ...
    @Override
    public int getOrder() {
        return GatewayFilterConstant.FILTER_API_WHITE_LIST_ORDER;
    }

    @Override
    protected boolean shouldSkip(ServerWebExchange exchange) {
        return Boolean.FALSE.equals(property.getEnabled());
    }
    ...
    private void markWhited(ServerWebExchange exchange) {
        ReactorUtil.setAttribute(exchange, GatewayWebExchangeConstant.WHITED_FLAG, true);
    }
    ...
    private Mono<Set<String>> getApiWhiteList() {
        return Mono.fromFuture(() -> cache.get(GatewayCacheConstant.CACHE_KEY_ALL))
            ...
            .onErrorResume(e -> {
                // 降级使用同步缓存
                Set<String> fallback = cache.synchronous().getIfPresent(GatewayCacheConstant.CACHE_KEY_ALL);
    ...
    private Mono<Set<String>> loadApiWhiteList() {
        return mgtClient.listApi()
            ...
            .map(remoteList -> {
                ...
                List<String> local = property.getIgnoredApi();
     ...
     * @param message 消息
     */
    public void refreshWhiteList(MessageBaseDTO<ApiWhiteListChangeVO> message) {
        ...
        ApiWhiteListChangeVO data = message.getData();

        cache.synchronous().asMap().compute(GatewayCacheConstant.CACHE_KEY_ALL, (key, old) -> {
