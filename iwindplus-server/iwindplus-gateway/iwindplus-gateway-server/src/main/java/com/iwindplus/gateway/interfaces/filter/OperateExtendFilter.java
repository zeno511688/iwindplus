...
import com.iwindplus.gateway.infrastructure.configuration.OperateExtendProperty;
...
public class OperateExtendFilter extends BaseGatewayFilter {
    ...
    private final OperateExtendProperty operateExtendProperty;
    private final LogProperty logProperty;
    private final MgtClient mgtClient;

    public OperateExtendFilter(
        OperateExtendProperty operateExtendProperty,
        LogProperty logProperty,
        MgtClient mgtClient) {
        this.operateExtendProperty = operateExtendProperty;
    ...
    @Override
    public int getOrder() {
        return GatewayFilterConstant.FILTER_OPERATE_EXTEND_ORDER;
    }

    @Override
    protected boolean shouldSkip(ServerWebExchange exchange) {
        // 开关 + 白名单
        if (GatewayUtil.shouldSkip(exchange, operateExtendProperty.getEnabled())) {
            return true;
        }

        // 忽略 API
        String path = exchange.getRequest().getPath().value();
        if (CollUtil.isNotEmpty(operateExtendProperty.getIgnoredApi())
            && PathMatchUtil.match(operateExtendProperty.getIgnoredApi(), path)) {
            return true;
        }
        ...
        if (user != null
            && CollUtil.isNotEmpty(operateExtendProperty.getIgnoredUser())
            && operateExtendProperty.getIgnoredUser().contains(user.getUsername())) {
            return true;
        }
    ...
    private Mono<Void> checkExtendFunction(ServerWebExchange exchange, UserBaseVO user) {
        ...
        boolean needGa = matchAny(operateExtendProperty.getIncludeGaApi(), path);
        boolean needMail = matchAny(operateExtendProperty.getIncludeMailApi(), path);
        boolean needSms = matchAny(operateExtendProperty.getIncludeSmsApi(), path);
        boolean needYubikey = matchAny(operateExtendProperty.getIncludeYubikeyApi(), path);
