...
import com.iwindplus.gateway.infrastructure.configuration.ApiSignProperty;
...
public class ApiSignFilter extends BaseGatewayFilter {
    ...
    @Override
    public int getOrder() {
        return GatewayFilterConstant.FILTER_API_SIGN_ORDER;
    }
    ...
    protected boolean shouldSkip(ServerWebExchange exchange) {
        ...
        List<String> custom = property.getIgnoredApi();
    ...
    private Mono<Void> checkApiSign(ServerWebExchange exchange) {
        return buildApiSign(exchange)
            .flatMap(dto -> getApiSign(dto.getAccessKey())
                .flatMap(apiSign -> {
                    if (apiSign == null) {
                        return GatewayUtil.asyncPublishErrorLog(exchange, logProperty, new BizException(BizCodeEnum.ACCESS_KEY_NOT_EXIST));
                    }
                    dto.setSecretKey(apiSign.getSecretKey());
                    dto.setTimeout(Duration.ofSeconds(apiSign.getTimeout()));

                    if (!ApiSignUtil.verifySign(dto)) {
                        return GatewayUtil.asyncPublishErrorLog(exchange, logProperty, new BizException(BizCodeEnum.INVALID_SIGN));
                    }

                    if (Boolean.TRUE.equals(property.getEnabledExecuteOnlyOnce())) {
                        // 防重放攻击
                        final HttpMethod method = exchange.getRequest().getMethod();
                        if (!HttpMethod.GET.equals(method)) {
                            String key = RedisConstant.REPEAT_SUBMIT_KEY_PREFIX + dto.getSign();
                            return redissonExecutor.repeatSubmit()
                                .executeReactive(
                                    key,
                                    property.getOnlyCheckOnceTtl(),
                                    Mono::empty
                                );
                        }
                    }
    ...
    private Mono<ApiSignVerifyDTO> buildApiSign(ServerWebExchange exchange) {
        ...
        if (CharSequenceUtil.isBlank(timestamp)) {
            return GatewayUtil.asyncPublishErrorLog(exchange, logProperty, new BizException(BizCodeEnum.TIMESTAMP_NOT_EXIST));
        }
        if (CharSequenceUtil.isBlank(nonce) || nonce.length() < NumberConstant.NUMBER_TEN) {
            return GatewayUtil.asyncPublishErrorLog(exchange, logProperty, new BizException(BizCodeEnum.INVALID_NONCE));
        }
        if (CharSequenceUtil.isBlank(sign)) {
            return GatewayUtil.asyncPublishErrorLog(exchange, logProperty, new BizException(BizCodeEnum.SIGN_NOT_EXIST));
        }
