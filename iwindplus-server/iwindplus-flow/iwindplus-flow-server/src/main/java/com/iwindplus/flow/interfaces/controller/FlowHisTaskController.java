...
import com.iwindplus.flow.application.query.task.FlowHisTaskQueryService;
...
public class FlowHisTaskController extends BaseController {
    ...
    private final FlowHisTaskQueryService flowHisTaskQueryService;
    ...
    public ResultVO<IPage<FlowHisTaskPageVO>> approvalRecord(@Validated FlowHisTaskSearchDTO entity) {
        if (Boolean.TRUE.equals(entity.getOnlyMine())) {
            entity.setUserId(this.getUserInfo().getUserId());
        }
        IPage<FlowHisTaskPageVO> data = this.flowHisTaskQueryService.approvalRecordPage(entity);
