...
CREATE TABLE `chat_group` (
  ...
  `status` int(10) unsigned NOT NULL DEFAULT 1 COMMENT '状态（0：封禁，1：正常）',
  ...
  `limit_num` int(10) unsigned NOT NULL DEFAULT 1 COMMENT '群限制数量',
  `show_nick_name_flag` tinyint(3) unsigned NOT NULL DEFAULT 1 COMMENT '是否显示群成员昵称（0：否，1：是）',
  `edit_avatar_flag` tinyint(3) unsigned NOT NULL DEFAULT 0 COMMENT '是否禁止修改群头像（0：否，1：是）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COMMENT='聊天群表';
...
CREATE TABLE `chat_group_user` (
  ...
  `leader_flag` tinyint(3) unsigned NOT NULL DEFAULT 0 COMMENT '是否群主（0：否，1：是）',
  `join_type` int(10) unsigned NOT NULL DEFAULT 0 COMMENT '加入类型（0:邀请，1：扫码）',
  `agree_flag` tinyint(3) unsigned NOT NULL DEFAULT 1 COMMENT '是否同意（0：拒绝，1：同意）',
  `seq` int(10) unsigned NOT NULL DEFAULT 1 COMMENT '排序号',
  `chat_group_id` bigint(20) unsigned NOT NULL DEFAULT 0 COMMENT '聊天群主键',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COMMENT='聊天群用户表';
...
CREATE TABLE `user_friend` (
  ...
  `version` int(10) unsigned NOT NULL DEFAULT 0 COMMENT '乐观锁（处理并发）',
  `remark` varchar(255) NOT NULL DEFAULT '' COMMENT '备注',
  `status` tinyint(3) unsigned NOT NULL DEFAULT 0 COMMENT '好友状态（0：待确认，1：已通过，2：已拒绝）',
  `user_id` bigint(20) unsigned NOT NULL DEFAULT 0 COMMENT '用户主键',
  `user_avatar` varchar(255) NOT NULL DEFAULT '' COMMENT '用户头像',
  `user_nick_name` varchar(50) NOT NULL DEFAULT '' COMMENT '用户昵称',
  `friend_id` bigint(20) unsigned NOT NULL DEFAULT 0 COMMENT '好友主键',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COMMENT='用户好友表';
