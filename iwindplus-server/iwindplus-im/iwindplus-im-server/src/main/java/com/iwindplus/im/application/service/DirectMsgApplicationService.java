...
import com.iwindplus.im.application.service.dto.DirectMsgDTO;
...
public class DirectMsgApplicationService {
    ...
    public boolean save(DirectMsgDTO entity) {
        ...
        entity.setSeq(this.directMsgRepository.getNextSeq(orgId, userId));
        ...
        this.directMsgRepository.save(model);
    ...
    public boolean edit(DirectMsgDTO entity) {
        DirectMsgDO data = this.directMsgRepository.getById(entity.getId());
        if (Objects.isNull(data)) {
            throw new BizException(BizCodeEnum.DATA_NOT_EXIST);
        }
        final DirectMsgDO model = BeanUtil.copyProperties(entity, DirectMsgDO.class);
        return this.directMsgRepository.updateById(model);
    }
    ...
    public boolean removeByIds(List<String> ids) {
        List<DirectMsgDO> list = this.directMsgRepository.listById(ids);
        if (CollUtil.isEmpty(list)) {
            throw new BizException(BizCodeEnum.DATA_NOT_EXIST);
        }
        this.directMsgRepository.removeByIds(ids, false);
        return Boolean.TRUE;
    }
