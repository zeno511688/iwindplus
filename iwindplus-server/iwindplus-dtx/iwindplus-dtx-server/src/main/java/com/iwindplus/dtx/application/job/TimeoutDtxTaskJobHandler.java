...
import com.iwindplus.dtx.common.enums.DtxTaskJobEnum;
...
@Component
@Slf4j
public class TimeoutDtxTaskJobHandler extends AbstractDtxTaskJobHandler {

    @Override
    public DtxTaskJobEnum support() {
        return DtxTaskJobEnum.TIMEOUT_JOB;
    }

    @Override
    protected boolean doExecute(List<TccGlobalTxVO> entityList) {
        log.info("分布式事务超时任务开始处理，size={}", entityList.size());
        ...
        for (TccGlobalTxVO tx : entityList) {
            try {
                ...
                failCount++;
                log.error("分布式事务超时任务异常，xid={}, error={}", tx.getXid(), e.getMessage(), e);
            }
        }

        log.info("分布式事务超时任务处理完成，总数={}, 成功={}, 失败={}", entityList.size(), successCount, failCount);
        return true;
    }
    ...
    private List<TccBranchTxDO> getBranches(String xid) {
        try {
            // 查询所有状态的分支事务
            return this.tccBranchTxQueryService.listByXid(xid,
