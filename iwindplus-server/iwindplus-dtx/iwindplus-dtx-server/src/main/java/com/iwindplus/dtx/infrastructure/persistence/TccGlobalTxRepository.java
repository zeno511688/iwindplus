...
import com.iwindplus.dtx.common.enums.DtxCodeEnum;
...
public class TccGlobalTxRepository extends CrudRepository<TccGlobalTxMapper, TccGlobalTxDO> {
    ...
    public boolean updateStatusById(Long id, GlobalTxStatusEnum from, GlobalTxStatusEnum to) {
        return this.updateStatus(Wrappers.<TccGlobalTxDO>lambdaUpdate()
            .eq(TccGlobalTxDO::getId, id)
            .eq(TccGlobalTxDO::getStatus, from), to, null, null);
    }
    ...
    public boolean updateStatusById(
        String xid,
        GlobalTxStatusEnum from,
        GlobalTxStatusEnum to,
        Integer retryCount, Long nextRetryTime) {
        return this.updateStatus(Wrappers.<TccGlobalTxDO>lambdaUpdate()
            .eq(TccGlobalTxDO::getXid, xid)
            .eq(TccGlobalTxDO::getStatus, from), to, retryCount, nextRetryTime);
    }
    ...
    public boolean updateStatusByXidAndMultiFrom(String xid, List<GlobalTxStatusEnum> fromList, GlobalTxStatusEnum to) {
        return this.updateStatus(Wrappers.<TccGlobalTxDO>lambdaUpdate()
            .eq(TccGlobalTxDO::getXid, xid)
            .in(TccGlobalTxDO::getStatus, fromList), to, null, null);
    }
    ...
    private boolean updateStatus(
        LambdaUpdateWrapper<TccGlobalTxDO> updateWrapper,
        GlobalTxStatusEnum to,
        Integer retryCount, Long nextRetryTime) {
        final TccGlobalTxDO update = TccGlobalTxDO
            .builder()
            .status(to)
     ...
     *
     * @param xid 全局事务ID
     * @return TccGlobalTxDO
     */
    public TccGlobalTxDO getByXid(String xid) {
        return super.getOne(Wrappers.lambdaQuery(TccGlobalTxDO.class)
            .eq(TccGlobalTxDO::getXid, xid));
    }
