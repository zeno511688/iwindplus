...
package com.iwindplus.dtx.domain.tcc;
...
import com.iwindplus.dtx.application.query.vo.TccBranchResultVO;
...
import com.iwindplus.dtx.common.enums.TxActionEnum;
import com.iwindplus.dtx.infrastructure.configuration.DtxTaskProperty;
...
public class TccCoordinatorImpl implements TccCoordinator {
    ...
    @Resource
    private TccGlobalTxRepository tccGlobalTxRepository;

    @Resource
    private TccBranchTxRepository tccBranchTxRepository;

    @Resource(name = DtxConstant.THREAD_POOL_BEAN_NAME)
    private DtpExecutor threadPoolExecutor;

    @Resource
    private HttpClientExecuteHandlerFactory factory;
    ...
    public String begin(String bizType, Long timeoutSeconds) {
        ...
        this.tccGlobalTxRepository.getXidExist(xid);

        final TccGlobalTxDO model = BeanUtil.copyProperties(entity, TccGlobalTxDO.class);
        model.setEnv(SpringUtil.getActiveProfile());
    ...
    public boolean trySuccess(Long id) {
        return this.tccBranchTxRepository.updateStatusById(
            id,
    ...
    private boolean executeGlobalTx(String xid, TxActionEnum action) {
        TccGlobalTxDO data = this.tccGlobalTxRepository.getByXid(xid);
        ...
        TccGlobalTxDO latest = null;
        if (this.tccGlobalTxRepository.updateStatusByXidAndMultiFrom(
            xid,
            action.getRetryableStatusList(),
            action.getProcessingStatus()
        )) {
            latest = this.tccGlobalTxRepository.getByXid(xid);
        }
    ...
    private boolean executeBranches(String xid, Long timeoutSeconds, TxActionEnum action) {
        List<TccBranchTxDO> branches =
            this.tccBranchTxRepository.listByXid(xid, action.getBranchSourceStatusList());
    ...
    private TccBranchResultVO checkAndLockBranch(
        ...
        TccBranchTxDO latest = this.tccBranchTxRepository.getById(branch.getId());
        ...
        // 尝试获取锁（原子更新状态）
        boolean locked = this.tccBranchTxRepository.updateStatusByIdAndMultiFrom(
            branch.getId(),
    ...
    private TccBranchResultVO invokeRemote(
        ...
        try {
            ...
            ResultVO<Boolean> res = factory.getHandler(HttpClientTypeEnum.REST_CLIENT)
                .post(
                    url,
                    branch.getPayload(),
                    null,
    ...
    private TccBranchResultVO handleSuccess(
        ...
        TccBranchTxDO current = this.tccBranchTxRepository.getById(branch.getId());
        ...
        this.tccBranchTxRepository.updateStatusById(
            branch.getId(),
            action.getBranchProcessingStatus(),
    ...
    private TccBranchResultVO validateDbState(TccBranchResultVO result) {
        TccBranchTxDO latest =
            this.tccBranchTxRepository.getById(result.getBranchId());
    ...
    private void scheduleRetry(TccGlobalTxDO data, TxActionEnum action) {
        TccGlobalTxDO latest = this.tccGlobalTxRepository.getByXid(data.getXid());
        ...
        this.tccGlobalTxRepository.updateStatusById(
            data.getXid(),
            action.getProcessingStatus(),
    ...
    private boolean finalizeGlobalTx(String xid, TxActionEnum action) {
        boolean result = this.tccGlobalTxRepository.updateStatusByXid(
            xid,
            action.getProcessingStatus(),
        ...
        if (result) {
            ...
            if (Boolean.TRUE.equals(this.property.getEnabledSuccessDelete())) {
                this.tccGlobalTxRepository.deleteByXid(xid, this.property.getEnabledSuccessRealDelete());
            }
        } else {
