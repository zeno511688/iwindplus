...
| `iwindplus-base-address`                                                   | 基于 HTTP 客户端的第三方地址服务接口集成      |
| `iwindplus-base-sumsub`                                                    | 基于 HTTP 客户端的第三方sumsub服务接口集成  |
...
| `iwindplus-base-async-task`                                                | 本地事务消息、异步任务和失败重试             |
...
| `iwindplus-base-monitor` / `iwindplus-base-logging-alert`                            | 链路上下文、监控埋点和日志告警              |
...
| `iwindplus-base-export-task`                                               | 支持 Excel、PDF、Docx 等多种格式的导出   |
