...
(
    ...
    `status`             int unsigned     NOT NULL DEFAULT 0 COMMENT '状态（0：待执行，10：执行中，20：等待中，30：成功，40：失败，50：废弃）',
...
-- ----------------------------
-- Table structure for async_task_sub
-- ----------------------------
DROP TABLE IF EXISTS `async_task_sub`;
CREATE TABLE `async_task_sub`
(
    `id`                 bigint unsigned  NOT NULL COMMENT '主键',
    `created_timestamp`  bigint unsigned  NOT NULL DEFAULT (unix_timestamp() * 1000) COMMENT '创建时间戳',
    `created_by`         varchar(50)      NOT NULL DEFAULT '' COMMENT '创建人',
    ...
    `version`            int unsigned     NOT NULL DEFAULT 0 COMMENT '乐观锁（处理并发）',
    `remark`             varchar(255)     NOT NULL DEFAULT '' COMMENT '备注',
    `status`             int unsigned     NOT NULL DEFAULT 0 COMMENT '状态（0：待执行，10：执行中，20：等待中，30：成功，40：失败）',
    `biz_name`           varchar(50)      NOT NULL DEFAULT '' COMMENT '业务名称',
    `biz_key`            varchar(50)      NOT NULL DEFAULT '' COMMENT '业务key，例如 ORDER',
    `biz_type`           varchar(50)      NOT NULL DEFAULT '' COMMENT '业务类型，例如 ORDER_CREATE',
    `biz_number`         varchar(50)      NOT NULL DEFAULT '' COMMENT '业务流水号，例如订单号',
    ...
    `progress`           int unsigned     NOT NULL DEFAULT 0 COMMENT '进度比例（0-100）',
    `ext`                text             NOT NULL DEFAULT '' COMMENT '扩展字段（JSON格式）',
    `async_task_id`       bigint unsigned  NOT NULL DEFAULT 0 COMMENT '异步任务主键',
    PRIMARY KEY (`id`),
    KEY `idx_biz_number` (`biz_number`) COMMENT '普通索引（业务流水号）',
    KEY `idx_async_task_id_seq_status` (`async_task_id`, `seq`, `status`) COMMENT '复合索引（异步任务主键, 排序号, 状态）'
) COMMENT ='异步任务子表';
