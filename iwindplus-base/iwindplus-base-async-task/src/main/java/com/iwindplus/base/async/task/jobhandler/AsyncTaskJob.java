...
package com.iwindplus.base.async.task.jobhandler;
...
public class AsyncTaskJob {
    ...
    public void jobExecute() {
        ...
        XxlJobHelper.log("异步任务开始执行，参数={}，开始时间={}，分片索引={}，分片总数={}", jobParam, start, shardIndex, shardTotal);

        int failedJobs = 0;
        final AsyncTaskJobEnum[] jobEnums = AsyncTaskJobEnum.values();
        for (AsyncTaskJobEnum entity : jobEnums) {
            try {
                factory.getJobHandler(entity).execute(shardIndex, shardTotal);
            } catch (Exception e) {
                failedJobs++;

                log.error("异步任务={}执行失败", entity.getDesc(), e);
                XxlJobHelper.log("异步任务={}执行失败", entity.getDesc(), e);
            }
        }

        if (failedJobs > 0) {
            XxlJobHelper.handleFail("异步任务部分失败，失败个数=" + failedJobs);
            return;
        }

        final long endTimeMillis = System.currentTimeMillis();
        XxlJobHelper.log("异步任务执行完成，总任务个数={}，结束时间={}，总执行毫秒数={}", jobEnums.length,
