...
package com.iwindplus.base.async.task.executor.impl;
...
import com.iwindplus.base.async.task.domain.dto.AsyncTaskCallbackBaseDTO;
...
import com.iwindplus.base.async.task.domain.dto.AsyncTaskSubmitBaseDTO;
...
import com.iwindplus.base.async.task.domain.vo.AsyncTaskSubSubmitVO;
...
public class AsyncTaskExecutorImpl implements AsyncTaskExecutor {
    ...
    public AsyncTaskSubmitVO submit(AsyncTaskSubmitDTO entity) {
        ...
        final AsyncTaskHandler handler = this.resolveTaskHandler(entity.getExecutorClass());
        Assert.notNull(handler, "task handler is not registered: " + entity.getExecutorClass().getName());
        ...
        // 保存数据
        final AsyncTaskSaveDTO param = BeanUtil.copyProperties(entity, AsyncTaskSaveDTO.class);
        param.setExecuteName(handler.getExecuteName());
        final AsyncTaskVO result = this.asyncTaskService.save(param);
    ...
    @Override
    public AsyncTaskSubmitVO submitGroup(AsyncTaskGroupSubmitDTO entity) {
        // 校验参数
        ...
        // 保存数据
        final AsyncTaskGrouSaveDTO param = BeanUtil.copyProperties(entity, AsyncTaskGrouSaveDTO.class);
        param.setExecuteName(handler.getExecuteName());

        List<AsyncTaskSubSaveDTO> subTasks = this.buildSubTasks(entity.getSubTasks());
        param.setSubTasks(subTasks);
        param.setSubTaskCount(subTasks.size());

        final AsyncTaskVO result = this.asyncTaskService.saveGroup(param);
    ...
    @Override
    public boolean callback(AsyncTaskCallbackDTO entity) {
        Assert.notNull(entity, "callback request must not be null");

        // 归一化回调结果
        this.normalizeCallbackResult(entity);
        // 定位主任务：优先 ID，其次 bizNumber
        final AsyncTaskVO task = this.getCallbackTask(entity);
        // 主任务存在时，终态直接幂等返回
        if (Objects.nonNull(task) && this.isFinished(task)) {
            log.warn(
                "asyncTask callback ignored, task already finished. id={} status={} bizNumber={}",
                task.getId(),
                task.getStatus(),
                task.getBizNumber()
            );
        ...
        // 构建子任务更新
        final List<AsyncTaskSubDO> subTaskUpdates = this.buildSubTaskUpdates(entity, task);
        // 构建主任务更新
        final AsyncTaskDO taskUpdate = this.buildTask(entity, task);

        // 主任务、子任务都不存在
        Assert.isTrue(
            Objects.nonNull(taskUpdate) || CollUtil.isNotEmpty(subTaskUpdates),
            "asyncTask callback target not found"
        );

        // 批量更新
        final boolean result = this.asyncTaskService.editCallbackBatch(taskUpdate, subTaskUpdates);
        if (!result) {
            return false;
        }
        ...
        if (CollUtil.isNotEmpty(subTaskUpdates)) {
            final Long asyncTaskId = subTaskUpdates.get(0).getAsyncTaskId();
            final AsyncTaskVO parentTask =
                this.asyncTaskService.getDetail(asyncTaskId);

            Assert.notNull(parentTask, "asyncTask not found");
    ...
    public void retryById(Long id) {
        ...
        this.retry(this.asyncTaskService.getDetail(id));
    }
    ...
    public void retryByBizNumber(String bizNumber) {
        ...
        this.retry(this.asyncTaskService.getDetailByBizNumber(bizNumber));
    }
    ...
    public boolean removeById(Long id) {
        ...
        return asyncTaskService.removeById(id, true);
    }
    ...
    public boolean removeByBizNumber(String bizNumber) {
        ...
        return asyncTaskService.removeByBizNumber(bizNumber, true);
    }

    private void dispatch(AsyncTaskVO entity) {
        this.asyncTaskBizProcessor.execute(entity);
    }
     ...
     */
    private void checkSubmitParam(AsyncTaskSubmitBaseDTO entity, boolean needCheckSubTasks) {
        ...
        if (entity instanceof AsyncTaskSubmitDTO asyncTaskSubmitDTO) {
            Assert.notNull(asyncTaskSubmitDTO.getExecutorClass(), "executorClass must not be null");
        } else if (entity instanceof AsyncTaskGroupSubmitDTO asyncTaskGroupSubmitDTO) {
            Assert.notNull(asyncTaskGroupSubmitDTO.getExecutorClass(), "executorClass must not be null");
            if (needCheckSubTasks) {
                Assert.notEmpty(asyncTaskGroupSubmitDTO.getSubTasks(), "subTasks must not be null");
            }
        }
    }

    private void checkSubSubmitParam(AsyncTaskSubSubmitDTO entity, Integer index) {
    ...
    private List<AsyncTaskSubSaveDTO> buildSubTasks(List<AsyncTaskSubSubmitDTO> subTasks) {
        final List<AsyncTaskSubSaveDTO> entities = new ArrayList<>(10);
        ...
        for (int index = 0; index < subTasks.size(); index++) {
            final AsyncTaskSubSubmitDTO subTask = subTasks.get(index);
            ...
            final AsyncTaskSubHandler handler = this.resolveSubTaskHandler(subTask.getExecutorClass());
            Assert.notNull(
                handler,
                "sub[" + index + "].task handler is not registered: " + subTask.getExecutorClass().getName()
            );
            ...
            final AsyncTaskSubSaveDTO entity = BeanUtil.copyProperties(subTask, AsyncTaskSubSaveDTO.class);
    ...
    private AsyncTaskSubmitVO buildSubmitResult(AsyncTaskVO param) {
        final AsyncTaskSubmitVO result = AsyncTaskSubmitVO
            .builder()
            .id(param.getId())
        ...
        if (CollUtil.isNotEmpty(param.getSubTasks())) {
            result.setSubTasks(BeanUtil.copyToList(param.getSubTasks(), AsyncTaskSubSubmitVO.class));
        }
        return result;
    }

    public void retry(AsyncTaskVO entity) {
        if (Objects.isNull(entity)) {
            return;
        }

        final AsyncTaskStatusEnum from = entity.getStatus();
        if (AsyncTaskStatusEnum.DISCARD != from) {
            throw new BizException(BizCodeEnum.CURRENT_STATUS_NOT_SUPPORT_RETRY, new Object[]{from});
        }

        final boolean status = this.asyncTaskService.editStatusById(AsyncTaskStatusEditDTO.builder()
            .id(entity.getId())
            .from(from)
    ...
    private boolean overrideCallback(AsyncTaskHandler handler) {
        try {
            final Class<?> executeCallback = handler.getClass()
                .getMethod(AsyncTaskConstant.METHOD_EXECUTE_CALLBACK, AsyncTaskVO.class).getDeclaringClass();
            return !AsyncTaskHandler.class.equals(executeCallback);
    ...
    private boolean overrideSubCallback(AsyncTaskSubHandler handler) {
        try {
            final Class<?> executeSubCallback = handler.getClass()
                .getMethod(AsyncTaskConstant.METHOD_EXECUTE_SUB_CALLBACK, AsyncTaskSubVO.class).getDeclaringClass();
            return !AsyncTaskSubHandler.class.equals(executeSubCallback);
        } catch (NoSuchMethodException ex) {
     ...
     * @param entity 回调通知对象
     */
    private void normalizeCallbackResult(AsyncTaskCallbackBaseDTO entity) {
        if (Objects.isNull(entity)) {
            return;
        }

        final Integer progress = entity.getProgress();
        final AsyncTaskCallbackResultEnum callbackResult = entity.getCallbackResult();
        ...
        if (Objects.isNull(callbackResult) && Objects.nonNull(progress) && progress >= NumberConstant.NUMBER_ONE_HUNDRED) {
            entity.setCallbackResult(AsyncTaskCallbackResultEnum.SUCCESS);
            return;
        }

        // SUCCESS 且无进度 → 100
        if (AsyncTaskCallbackResultEnum.SUCCESS.equals(callbackResult) && Objects.isNull(progress)) {
     ...
     */
    private Map<String, Object> buildCallbackResult(AsyncTaskCallbackBaseDTO entity) {
        ...
        Optional.ofNullable(entity.getCallbackResult())
            .ifPresent(callbackResult -> result.put(AsyncTaskConstant.CALLBACK_RESULT_KEY, callbackResult.name()));

        // 添加错误信息
        Optional.ofNullable(entity.getErrorMsg())
            .filter(CharSequenceUtil::isNotBlank)
            .ifPresent(errorMsg -> result.put(AsyncTaskConstant.CALLBACK_ERROR_MSG_KEY, errorMsg));
     ...
     * @return 主任务
     */
    private AsyncTaskVO getCallbackTask(AsyncTaskCallbackDTO entity) {
        AsyncTaskVO task = null;

        if (Objects.nonNull(entity.getId())) {
            task = this.asyncTaskService.getDetail(entity.getId());
        }

        if (Objects.isNull(task) && CharSequenceUtil.isNotBlank(entity.getBizNumber())) {
            task = this.asyncTaskService.getDetailByBizNumber(entity.getBizNumber());
        }
     ...
     * @return 子任务更新列表
     */
    private List<AsyncTaskSubDO> buildSubTaskUpdates(
        AsyncTaskCallbackDTO entity,
        AsyncTaskVO task) {
        final List<AsyncTaskSubCallbackDTO> subTasks = entity.getSubTasks();
        ...
        final List<String> bizNumbers = subTasks.stream()
            ...
            .map(AsyncTaskSubCallbackDTO::getBizNumber)
            .filter(CharSequenceUtil::isNotBlank)
            .distinct()
            .toList();
        ...
        // 一次查询全部子任务
        final List<AsyncTaskSubVO> subTaskList =
            this.asyncTaskSubService.listByBizNumbers(bizNumbers);

        final Map<String, AsyncTaskSubVO> subTaskMap = subTaskList.stream()
            .collect(Collectors.toMap(
                AsyncTaskSubVO::getBizNumber,
                Function.identity()
            ));
        ...
        if (Objects.nonNull(task)) {
            Assert.isTrue(
                subTaskList.stream().allMatch(sub -> Objects.equals(sub.getAsyncTaskId(), task.getId())),
     ...
     * @return 子任务更新列表
     */
    private List<AsyncTaskSubDO> buildSubTask(
        List<AsyncTaskSubCallbackDTO> subTasks,
        Map<String, AsyncTaskSubVO> subTaskMap) {
        final List<AsyncTaskSubDO> updates = new ArrayList<>(subTasks.size());

        for (int index = 0; index < subTasks.size(); index++) {
            final AsyncTaskSubCallbackDTO callback = subTasks.get(index);
            ...
            final AsyncTaskSubVO subTask = subTaskMap.get(callback.getBizNumber());
            Assert.notNull(
                subTask,
            ...
            // 已经成功的任务忽略重复回调
            if (AsyncTaskStatusEnum.SUCCESS.equals(subTask.getStatus())) {
                continue;
            }

            final AsyncTaskSubDO.AsyncTaskSubDOBuilder<?, ?> builder = AsyncTaskSubDO.builder()
                .id(subTask.getId())
                .asyncTaskId(subTask.getAsyncTaskId())
            ...
            if (Objects.nonNull(callback.getCallbackResult())) {
                builder.status(
                    AsyncTaskStatusEnum.valueOf(callback.getCallbackResult().name())
                );
            }
    ...
    private AsyncTaskDO buildTask(
        ...
        final AsyncTaskDO.AsyncTaskDOBuilder<?, ?> builder = AsyncTaskDO.builder()
            .id(task.getId())
        ...
        if (Objects.nonNull(entity.getCallbackResult())) {
            builder.status(AsyncTaskStatusEnum.valueOf(entity.getCallbackResult().name()));
        }
     ...
     * @param task 主任务
     */
    private boolean isFinished(AsyncTaskVO task) {
        final AsyncTaskStatusEnum status = task.getStatus();
        return AsyncTaskStatusEnum.SUCCESS.equals(status) || AsyncTaskStatusEnum.DISCARD.equals(status);
    }
}
