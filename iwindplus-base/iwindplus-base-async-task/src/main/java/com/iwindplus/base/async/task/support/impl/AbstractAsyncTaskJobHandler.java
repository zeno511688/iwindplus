/*
 *
 *  * Copyright (c) iwindplus Technologies Co., Ltd.2024-2030, All rights reserved.
 *
 *
 */

package com.iwindplus.base.async.task.support.impl;

import cn.hutool.core.collection.CollUtil;
import cn.hutool.extra.spring.SpringUtil;
import com.iwindplus.base.async.task.domain.dto.AsyncTaskShardSearchDTO;
import com.iwindplus.base.async.task.domain.property.AsyncTaskProperty;
import com.iwindplus.base.async.task.domain.vo.AsyncTaskVO;
import com.iwindplus.base.async.task.service.AsyncTaskService;
import com.iwindplus.base.async.task.support.AsyncTaskJobHandler;
import java.util.List;
import java.util.Objects;
import lombok.Getter;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;

/**
 * 抽象异步任务job助手策略实现类.
 *
 * @author zengdegui
 * @since 2025/11/29 23:12
 */
@Slf4j
@Getter
@RequiredArgsConstructor
public abstract class AbstractAsyncTaskJobHandler implements AsyncTaskJobHandler {

    private final AsyncTaskProperty property;
    private final AsyncTaskService asyncTaskService;

    /**
     * 执行.
     *
     * @param entityList 集合
     * @return boolean
     */
    protected abstract boolean doExecute(List<AsyncTaskVO> entityList);

    /**
     * 获取查询参数.
     *
     * @return AsyncTaskShardSearchDTO
     */
    protected abstract AsyncTaskShardSearchDTO buildJobSearchDTO();

    /**
     * 判断是否应该跳过该任务.
     *
     * @param entity 任务实体
     * @return true=跳过，false=不跳过
     */
    protected boolean shouldSkip(AsyncTaskVO entity) {
        return false;
    }

    @Override
    public void execute(Integer shardIndex, Integer shardTotal) {
        final Integer size = this.asyncTaskService.getSize();
        if (Objects.isNull(size) || size <= 0) {
            log.error("【{}】每轮捞取条数={}, 本轮不捞取，请检查每页条数配置", this.support(), size);
            return;
        }

        final AbstractAsyncTaskJobHandler proxy = SpringUtil.getBean(this.getClass());

        final AsyncTaskShardSearchDTO param = this.buildJobSearchDTO();
        param.setShardIndex(shardIndex);
        param.setShardTotal(shardTotal);
        param.setSize(size);

        long lastId = 0;
        int loop = 0;
        int total = 0;
        while (loop < this.property.getJob().getMaxLoopCount()) {
            if (Thread.currentThread().isInterrupted()) {
                break;
            }

            param.setLastId(lastId);

            final List<AsyncTaskVO> list = this.asyncTaskService.listByShard(param);
            if (CollUtil.isEmpty(list)) {
                break;
            }

            final boolean result = proxy.doExecute(list);
            if (!result) {
                log.warn("【{}】执行失败，提前结束本轮捞取，已处理={}", this.support(), total);

                break;
            }

            // 更新游标
            lastId = list.get(list.size() - 1).getId();
            loop++;
            total += list.size();
        }
    }

}
