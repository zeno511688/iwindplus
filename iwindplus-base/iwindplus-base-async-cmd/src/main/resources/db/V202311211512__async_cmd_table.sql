...
(
    ...
    `status`               int unsigned     NOT NULL DEFAULT 0 COMMENT '状态（0：待执行，10：执行中，10：异步等待，30：成功，40：失败）',
...
(
    ...
    `version`              int unsigned     NOT NULL DEFAULT 0 COMMENT '乐观锁（处理并发）',
    `remark`               varchar(255)     NOT NULL DEFAULT '' COMMENT '备注',
    `status`               int unsigned     NOT NULL DEFAULT 0 COMMENT '状态（0：待执行，10：执行中，10：异步等待，30：成功，40：失败，50：废弃）',
    `biz_type`             varchar(50)      NOT NULL DEFAULT '' COMMENT '业务类型，例如 ORDER_CREATE',
