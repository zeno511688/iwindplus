# 日志告警模块（iwindplus-base-logging-alert）
...
<dependency>
    ...
    <artifactId>iwindplus-base-logging-alert</artifactId>
</dependency>
```
...
配置前缀为 `logging`，属性类为 `LoggingAlertProperty`。
...
| 配置项 | 默认值 | 说明 |
|---|---:|---|
| `logging.enabled` | `false` | 是否启用日志告警；建议显式设置为 `true` |
| `logging.owners` | 空 | 告警消息中的责任人列表，多个值使用列表配置 |
| `logging.exclude-patterns` | 空 | 排除正则表达式；日志消息命中任一表达式时不发送 |
| `logging.sample-rate` | `100` | 采样率，`100` 表示全部采样，`10` 表示约 10% |
...
| 配置项 | 默认值 | 说明 |
|---|---|---|
| `logging.channel.channel-type` | `FEI_SHU` | 告警渠道枚举值，实际可用值以 `AlertChannelTypeEnum` 为准 |
| `logging.channel.code` | 空 | 渠道配置编码，对应 `alert.feishu[].code` |
| `logging.channel.webhook-code` | 空 | Webhook 配置编码，对应 `alert.feishu[].webhooks[].code` |

Webhook 地址和密钥统一由 `iwindplus-base-alert` 的 `alert.feishu[].webhooks[]` 配置提供，本模块通过 `channel-type`、`code`、`webhook-code` 定位到具体配置，再调用 `sendWebhookMsg` 发送。渠道配置缺失或编码错误时，发送会失败并记录在 Appender 错误日志中。
...
| 配置项 | 默认值 | 说明 |
|---|---:|---|
| `logging.rate-limit.window-seconds` | `60` | 时间窗口（秒），用于计算令牌生成速率 |
| `logging.rate-limit.silence-seconds` | `300` | 超过最大次数后的静默时间，单位秒 |
| `logging.rate-limit.max-requests` | `10` | 桶容量（时间窗口内允许的最大告警次数） |
| `logging.rate-limit.cache-size` | `1000` | 限流 Key 的最大缓存数量 |
...
| 配置项 | 默认值 | 说明 |
|---|---:|---|
| `logging.stack.max-length` | `5000` | 堆栈最大字符数；小于等于 0 表示不限制 |
| `logging.stack.max-frames` | `50` | 每个异常最多展示的堆栈帧数；小于等于 0 表示不限制 |
...
**实现原理：**
- `LoggingAlertAppender.append()` 方法中检查 `property.getEnabled()` 状态
- 配置刷新后，`LoggingAlertProperty` Bean 自动更新
- 下次日志输出时立即生效
...
配置开启后，`LoggingAlertConfiguration` 会自动创建并启动 `LoggingAlertAppender`，挂载到 Logback Root Logger。应用关闭时会自动从 Root Logger 移除并停止 Appender，不需要额外的 Logback XML 配置。

`logging.enabled` 未设置时，是否创建配置由当前自动配置条件和版本代码决定；为了避免环境差异，生产环境建议明确配置为 `true` 或 `false`。
...
- **动态配置**：通过 Nacos 配置中心修改 `logging.enabled` 可立即生效，无需重启服务。
...
2. 检查 `logging.channel` 的 `channel-type`、`code`、`webhook-code` 是否配置正确
3. 检查 `alert.feishu[].webhooks[]` 中对应的 Webhook 配置是否存在
4. 检查日志级别是否为 ERROR
5. 检查是否被 `exclude-patterns` 排除
6. 检查是否被限流（查看日志中的限流信息）
